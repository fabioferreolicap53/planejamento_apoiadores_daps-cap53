...
const HistoryView: React.FC<HistoryViewProps> = ({ onNavigate, plans, onEdit, onDelete, profile }) => {
  ...
  const filteredPlans = plans.filter(p => {
    const matchesSearch =
      parseLinhaCuidado(p.linha_cuidado).some(l => l.toLowerCase().includes(search.toLowerCase())) ||
    ...
    const matchesLinha = filterLinha === 'Todos' || parseLinhaCuidado(p.linha_cuidado).includes(filterLinha);
                ...
                <table ref={tableRef} className="w-full text-left border-collapse">
                  <thead className="sticky top-0 z-20">
                    <tr className="bg-[#f9fafb] dark:bg-gray-800/95 border-b border-[#e5e7eb] dark:border-gray-800 text-left backdrop-blur-md">
                      ...
                      <th className="py-3 px-6 text-xs font-semibold uppercase tracking-wider text-[#617589] bg-[#f9fafb] dark:bg-gray-800">Linhas de Cuidado</th>
                          ...
                          <div className="flex items-center gap-3">
                            <div className={`h-8 w-8 rounded-full flex items-center justify-center text-xs font-bold bg-blue-100 text-blue-700 shrink-0`}>
                              {(() => {
                                ...
                                return primeiraLinha.substring(0, 2).toUpperCase();
                              })()}
                            </div>
                            <div className="flex flex-col">
                              {(() => {
                                ...
                                return (
                                  <>
                                    ...
                                    {linhas.length > 2 && (
                                      <div className="text-xs text-gray-500">+{linhas.length - 2} mais</div>
                                    )}
                                  </>
                                );
                              })()}
